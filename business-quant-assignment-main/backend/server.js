...
app.get('/api/ticker=:ticker/columns=:columns', async (req, res) => {
  const ticker = req.params.ticker.toUpperCase();
  const columns = req.params.columns.split(',').map(column => column.trim()).join(',');
...
app.get('/api/ticker=:ticker/columns=:columns/period=:period', async (req, res) => {
  const ticker = req.params.ticker.toUpperCase();
  const columns = req.params.columns.split(',').map(column => column.trim()).join(',');
